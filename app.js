...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (user && await bcrypt.compare(password, user.password)) {
      return res.json({ msg: "exist", user: user.name, role: user.role }); // Ensure role is included here
    } else {
...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    if (password === cpassword) {
      ...
      await collection.create({ name, email, password: hashedPassword, dob, linkedIn });
...
app.post('/jobs', upload.single('companyLogo'), async (req, res) => {
  try {
    if (!req.file) {
      throw new Error('Company logo file is missing');
    }
...
app.get('/jobbs', async (req, res) => {
  try {
    const jobs = await Job.find({companyId:req.body._id}); // Adjust according to your model
    res.json(jobs);
  } catch (error) {
...
app.post('/apply', upload.single('resume'), async (req, res) => {
  try {
    ...
    const job = await Job.findById(formData.jobId); // Get the job details using jobId
    if (!job) {
